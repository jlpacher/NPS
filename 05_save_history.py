...
def save_history_bq():
    ...
    nps_real,
    nps_fcst
    FROM `mydataset.MYTABLE_TEMP.nps_forecast_mes`
    )
    ;

    -- salvar predicoes no bq
    -- primeira execucao somente (demais, deixar comentado)
    -- CREATE OR REPLACE TABLE `mydataset.MYTABLE_PROD.NPS_PREDICAO` AS (
    --   SELECT * FROM `mydataset.MYTABLE_TEMP.nps_forecast_semana_corrigido`
    -- ORDER BY data_atualizacao, id)
    -- ;
    -- CREATE OR REPLACE TABLE `mydataset.MYTABLE_PROD.NPS_MENSAL_DASHBOARD` AS (
    ...
    DELETE FROM `mydataset.MYTABLE_PROD.NPS_PREDICAO` WHERE data_atualizacao IN (
      SELECT data_atualizacao FROM `mydataset.MYTABLE_TEMP.nps_forecast_semana_corrigido`)
    ;
    DELETE FROM `mydataset.MYTABLE_PROD.NPS_MENSAL_DASHBOARD` WHERE data_atualizacao IN (
      SELECT data_atualizacao FROM `mydataset.MYTABLE_TEMP.nps_forecast_mes_corrigido`)
    ;
    -- demais execucoes: adiciona dados
    INSERT INTO `mydataset.MYTABLE_PROD.NPS_PREDICAO` 
    SELECT * FROM `mydataset.MYTABLE_TEMP.nps_forecast_semana_corrigido`
    ORDER BY data_atualizacao, id
    ;
    INSERT INTO `mydataset.MYTABLE_PROD.NPS_MENSAL_DASHBOARD` 
    SELECT * FROM `mydataset.MYTABLE_TEMP.nps_forecast_mes_corrigido`
    ORDER BY 1,2
    ; 
    -- cria tabela para dashboard predicao semanal
    CREATE OR REPLACE TABLE `mydataset.MYTABLE_PROD.NPS_SEMANAL_DASHBOARD` AS (
...
def main(folder, model_name):
    ...
    print('mydataset.MYTABLE_PROD.NPS_SEMANAL_DASHBOARD')
    print()
